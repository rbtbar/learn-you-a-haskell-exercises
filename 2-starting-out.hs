...
{-
 ...
 -}
duplicate :: [a] -> [a]
duplicate xs = concat [replicate 2 x | x <- xs]
...
{-
 ...
 -}
ziplike :: [a] -> [b] -> [(a,b)]
ziplike xs ys = if min (length xs) (length ys) == 0 then [] else [(xs !! i, ys !! i) | i <- [0..(min (length xs) (length ys) - 1)]]

-- Split a list l at k-th element into a tuple: The first part up to and including k-th element, the second part after k-th element
-- OLD: Split a list l at element k into a tuple: The first part up to and including k, the second part after k
