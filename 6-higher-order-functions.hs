...
--          sumInts 1 3 = 6
sumInts :: Int -> Int -> Int
sumInts a b =  b + if b > a then sumInts a (b-1) else 0
...
sumSquares a b = sq b + if b > a then sumSquares a (b-1) else 0
...
higherOrderSum intApplication a b = if b > a then intApplication b + higherOrderSum intApplication a (b-1) else intApplication a
...
higherOrderSequenceApplication f op z a b
  | a > b = z
  | a == b = op a
  | otherwise = op b `f` higherOrderSequenceApplication f op z a (b-1)


-- Define a factorial method using the higherOrderSequenceAppliction
hoFactorial :: Int -> Int
hoFactorial = higherOrderSequenceApplication (*) id 0 1
